function [rout,J,CV] = CVmodelNL(pars,data)
...
while notdone && NC < 100  
    ...
    M = eye(7);
    M(7,7) = 0;
    options=odeset('RelTol',REL_TOL, 'AbsTol',ODE_TOL, 'Mass', M, 'MassSingular','yes');
    sol = ode15s(@modelBasic_VVINL,tdc,Init,options,pars,tdc(1),data); %solves the ODE uses modelBasic
    sols= deval(sol,tdc);
    ...
    for w = 1:length(tdc) 
        ...
        espt(w) = ElastanceDriver(tdc(w)-tdc(1),Tclv,Trlv); % Septal elastance
    end

    % Ventricles
    Vrvf = (Vrv - VvU) + Vspt; % Right ventricle freewall pressure
    Vlvf = (Vlv - VvU) - Vspt; % Left ventricle freewall pressure

  
    % Non-Linear Pressure Equations
    prv = erv'*EMrv.*Vrvf + (1-erv')*P0rv.*(exp(Lrv*Vrvf)-1); % Right ventricle freewall pressure
