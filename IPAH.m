function [pars,Init,low, hi,data] = IPAH
...
% Patient-specific total blood volume
% Body Surface Area
BSA   = 1.85; 
...
% Systemic Veins (mmHg)
psv    = 6;  % mean systemic pressure

% Pulmonary arteries (mmHg)
pSpa  = 80;      % systolic pulmonary artery pressure
ppa   = 50;      % mean pulmonary arterial pressure
pDpa  = (ppa-1/3*pSpa)*3/2;  % diastolic pulmonary arterial pressure

% Pulmonary Veins (mmHg)
ppv   = 8;    % mean pulmonary vein pressure 
...
VMrv = 200;       % Max right ventricular volume 
...
Vpv  = 0.125*TotalVol; % Pulmonary vein volume (12.5%)
...
% Nonlinear model (lambda's) ventricles
...
EMspt  = 15*EMlv;          % Septum FW elstnce (mmHg/mL) 
